...
module.exports = class Cli {
   ...
   */
  async parseToYargs({command, describe, options = {}, run}) {
    const handler = async argv => {
      ...
      this.debug('%o starting command with handler options %o', command, data);

      // run our pre command hook
      await this.runHook('prerun', {id: argv._[0], data, cli: this});

      // if run is not a function then we need to set it
      if (run instanceof Function === false) {
        const task = this.tasks
          .filter(task => task.name === command)
          .map(task => require(task.file)(this))
      ...
      let result;
      try {
        result = await run(data.options, {
          app: this.app,
          context: this.context,
          ctx: this.ctx,
          debug: this.debug.extend(`task:${command}`),
  ...
  async run(argv = process.argv.slice(2), options = {}) {
    ...
    if (fs.existsSync(landofilePath)) {
      ...
      // @TODO: or should we use product.getComponent('core.app')?
      const App = require(minstrapper.app);
      // override some default static properties
      App.debug = this.debug.contract(-1).extend(App.name);
      const app = new App({landofile: landofilePath, config: this.product.config, plugins: this.product.getPlugins()});
    ...
    this.help = this.getHelp(this.tasks, [this.product, this]);
