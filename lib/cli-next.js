...
// core-next stuff
const gos = require('@lando/core-next/utils/get-object-sizes');
const nmp = require('@lando/core-next/utils/normalize-manifest-paths');
...
module.exports = class Cli {
  ...
  constructor({
    ...
    this.debug('instantiated cli with cache %o and hooks %o', this.cache, gos(this.hooks));
  }
  ...
  async run(argv = process.argv.slice(2), options = {}) {
    ...
    if (fs.existsSync(appfilePath)) {
      ...
      this.app = new App({appfile: appfilePath, config: this.product.config, plugins: this.product.plugins});
    }
