...
const debug = require('debug')('lando:@lando/cli@3:next');
...
const {Config} = require('@oclif/core/lib/config');
...
module.exports = class Cli {
  // TODO: make this the minstrapper config from hyperdrive?
  constructor({
    prefix = 'LANDO',
  ...
  defaultConfig(appConfig = {}) {
    ...
    return {
      ...
      pluginDirs: [
        ...
        path.join(srcRoot, 'node_modules', '@lando', 'core-next'),
  ...
  async run(argv = process.argv.slice(2), options = {}) {
    debug('starting %o with %o and %o', '@lando/cli@3:next:run', argv, options);
    ...
    config.runHook = async (event, data, handler = config.hookError) => {
    ...
    // @TODO: lets rework run and the hooks a bit? and hookhandler
    // we should try to replicate OCLIF in run so only init/prerun/postrun
    // we should use init for more events but we can scale that back?
    ...
    */
    // @TODO: we need to actually load plugins first?

    // @TODO: we currently cannot continue without TASKZ, lets at least add it back in?
    //        can we load these with just lando3 stuff or should we load them our way and instantiate them into lando?
    //
    // @TODO: purge stuff in run()?


    // @TODO:
      // final event to redump task and plugin caches?
    ...
    // @TODO: switch to hookHandler for better and more consistent error thingy?
    // @TODO: should we add hookhandler to this?
    if (channel || experimental || secretToggle) {
      console.error('--channel, --experimental and --secret-toggle are no longer valid flags');
    }

    // clear tasks cache if forced
    if (clear) {
      this.clearTaskCaches();
      console.error('Lando has cleared the tasks cache!');
    }

    // Initialize
    this.init(yargs, tasks, config, userConfig);

    // @TODO: should lando tasks be CLI only? OCLIF commands are.
    // @TODO: ADD IN PRERUN HOOK?
    // @TODO: ADD IN POSTRUN HOOK?
  };
};
