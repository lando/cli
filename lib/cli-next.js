...
module.exports = class Cli {
  ...
  async run(argv = process.argv.slice(2), options = {}) {
    ...
    // handle .js ambiguity eg basename usage in getCompnent and apply to task loading as well
    // fix double cache loading in file-storage?
    ...
    if (clear) {
      // rebuild the lando registry
      lando.rebuildRegistry();
      // if we have an app then rebuild that as well
