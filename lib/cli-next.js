...
module.exports = class Cli {
  ...
  constructor({
    ...
    // select correct storage backend
    const StorageBackend = cache ? FileStorage : NoStorage;
    // id
    ...
    if (!cache && fs.existsSync(cacheDir)) FileStorage.flush(cacheDir, debug.extend('#flush'));
  }
  ...
  async parseToYargs({command, describe, options = {}, run}, config = {}) {
    const handler = async argv => {
      ...
      this.debug('command %o starting with handler options %o', command, data);
