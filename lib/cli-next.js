...
const FileStorage = require('@lando/core-next/components/file-storage');
...
module.exports = class Cli {
  ...
  constructor({cacheDir = os.tmpdir(), hooks = [], id} = {}) {
    this.id = id || path.basename(process.argv[1]) || 'lando';
    // add private cache storage
    this.#_cache = new FileStorage(({debugspace: 'cli', dir: cacheDir}));
    // debug
    this.debug = require('debug')(`${this.id}:@lando/cli`);
    // hooks
    this.hooks = hooks;
  ...
  getHooks(ctx, reinit = false) {
    ...
    // get the hooks
    // @NOTE: should we allow hooks in config?
    const hooks = require('@lando/core-next/utils/get-manifest-array')('hooks', ctx.getPlugins())
      .map(group => ({...group, hooks: group.data.cli}));
  ...
  getTasks(ctx, args, reinit = false) {
    ...
    // get the list
    const list = Object.entries(require('@lando/core-next/utils/get-manifest-object')('tasks', ctx.config, ctx.getPlugins()))
  ...
  async parseToYargs({command, describe, options = {}, run}, config = {}) {
    const handler = async argv => {
      ...
      let result;
      try {
        const debug = require('debug')(`${this.id}:@lando/cli:command:${command}`);
        result = await run(data.options, {...config, debug});
      } catch (error) {
        err = error;
  ...
  async run(argv = process.argv.slice(2), options = {}) {
    this.debug('starting %o with %o and %o', '@lando/cli:run', argv, options);
    // add some color
    ...
    const product = new Product({config: minstrapper.config});
  ...
  async runHook(event, data) {
    return require('@lando/core-next/utils/run-hook')(event, data, this.hooks, {cli: this}, 'cli', this.exitError);
  };
