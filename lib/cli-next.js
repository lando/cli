...
module.exports = class Cli {
  ...
  async parseToYargs({command, describe, options = {}, run}, config = {}) {
    const handler = async argv => {
      ...
      let result;
      try {
        result = await run(data.options, {...config, cli: this, debug: this.debug.extend(`task:${command}`)});
      } catch (error) {
        err = error;
  ...
  async run(argv = process.argv.slice(2), options = {}) {
    ...
    this.cid = this.context.app ? this.app.id : product.id;
    ...
    for (const task of sortBy(help, 'command')) {
      ...
      else yargs.command(await this.parseToYargs(task, this));
    }
