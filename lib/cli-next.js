  ...
  async run(argv = process.argv.slice(2), options = {}) {
    ...
    const {app, context, lando, tasks} = config;
     // 1. do the same for apps
      // change config.app to config.minapp?

      // change app.id into a nanoid?
        // where should we persist the id? it probably should not be in cache? make just write it to dataDir once we can?
        // standardize things that need to be stable on ID instead of name, since that can change

    // optimize runtime
      // getRegistry and also make sure postRun clears both lando and app cache?
      // optimize task loading? v3 is faster from warm cache
