...
module.exports = class Cli {
  ...
  getTasks({id, noCache = false, registry = {}}, args) {
    ...
    if (registry.legacy && registry.legacy.tasks) {
      ...
      const help = list.map(task => require(task.file)(...args));
      // set and return
  ...
  async run(argv = process.argv.slice(2), options = {}) {
    ...
    // split cli/core/core-next tests up better, how do we dogfood for core?
      // CLI should basically just test the runtime splitter and maybe some unit tests?
      // core should build CLI from source with node_modules/@lando/core replaced?
      // core-next should try to use plugins: @lando/core ./../..?
      // might as well also get cli/docs testing set up as well
