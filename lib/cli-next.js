...
module.exports = class Cli {
  ...
  static product =  path.join(__dirname, '..', 'node_modules', '@lando', 'core-next', 'components', 'lando');
  ...
  async run(argv = process.argv.slice(2), options = {}) {
    ...
    const oclif = await Config.load(Object.keys(options).length === 0 ? __dirname : options);
    // normalize our argv
    ...
    const ohooks = oclif.plugins.map(plugin => ({
      id: plugin.name,
      hooks: plugin.hooks || [],
      name: plugin.name,
      root: plugin.root,
    ...
    // @TODO: switch this to get-hook-amount?
    this.debug('found %o early run oclif hooks', ohooks.length);
    this.hooks = this.hooks.concat(ohooks);
    ...
    const externalProduct = path.join(config.dataDir, 'plugins', '@lando', 'core-next', 'components', 'lando');
    const product = fs.existsSync(externalProduct) ? externalProduct : Cli.product;

    // set the default minstrapper product and config
    //
    // @NOTE: the minstrapper is a lightweight thing that is responsible for choosing a product and generating the config
    // needed to instantiate it. it
    const minstrapper = {
      product: path.join(coreBase, 'components', 'lando'),
    };
