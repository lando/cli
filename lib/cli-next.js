...
module.exports = class Cli {
   ...
   */
  async parseToYargs({command, describe, options = {}, run, level = 'app'}, config = {}) {
    const handler = async argv => {
      ...
      if (!run instanceof Function) run = config.tasks.find(task => task.command === command).run;
  ...
  async run(argv = process.argv.slice(2), options = {}) {
    ...
    const {app, context, lando, tasks} = config;
    // handle global args

    // flushCache(cache) on app and bootstrap
      // some sort of internal caching mechanism?
      // something to flush the internal cache?
      // switch to use filestorage?
      // getComponent/getClass need Config.wrapper?

    // 4. cache the result for speed
    // 5. regenerate cache on postrun
    // 6. --clear with id = debug and continue

    // rework basic non-app tasks to use new this.log stuff?
    // -vvv handling?

    // allow core to be overriden?
      // * how do we update lando/core?
      // * make `@lando/core` a plugin that contains plugins
        // needs to reflect internal/external core

    // 1. do the same for apps
    ...
    // 2. go through the plugins and figure out all the tasks
    // 3. merge in any tooling and recipe tasks
      // a special event for this?

    // load init task back in
    // need to figure out how to merge in CLI stuff from inits and sources and recipes?
    // make CLI into an object/module

    // 00. test thing outs?
    // 001. move lando bootstrap inside of CLI task.run?
      // rework init to put things inside task.run
      // maybe a legacy bootstrap?
    // 01. an init method that loads in some extra components and plguins/tasks etc
    // 02. check for cache files before init?

    // we are going to need to slowly implement task loading:

    // * merge in and run through tests?
    // * purge any code we dont need anymore?
    // 2. implement support for lando-tooling
    // 3. implement support for lando-recipes
    ...
    */
    ...
    if (clear) {
      context.app ? app.clearCache(['plugins', 'registry']) : lando.clearCache(['plugins', 'registry']);
      this.logToStderr('Lando has cleared the caches for %s!', context.app ? app.name : 'lando');
      this.exit();
    }
