...
const getMetricsEnv = () => {
  switch (true) {
    case _.has(process, 'env.CI'):
      return 'ci';

    case _.has(process, 'env.GITPOD_WORKSPACE_ID'):
    case _.has(process, 'env.CODESPACES'):
      return 'remote';
...
exports.buildConfig = options => {
  ...
  // Lando 4 will allow all non-bootstrap/compiletime config to be overriden in Landofiles'
