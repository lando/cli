...
const getHeader = (messages = []) => {
  ...
  // error
  if (types.includes('error')) return chalk.red(niceFont('Error!!!', 'Small Slant'));
  // warning
...
const getStyle = (type = 'tip') => {
  switch (type) {
    case 'error':
      return {color: 'red', figure: figures.error};
    case 'warn':
    case 'warning':
  ...
  switch (phase) {
    ...
    case 'report':
      const report = [
        ...
        `These ${italicize('may')} or ${italicize('may not')} prevent your app from working.`,
        '',
      ];
