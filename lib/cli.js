...
module.exports = class Cli {
  ...
  // runs listr tasks
  async processTasks(tasks, renderer = 'default') {
    const {Manager} = require('listr2');
    const rendererOptions = {collapseSubtasks: false, level: 1, suffixRetries: false, showErrorMessage: false};
    const listrOptions = {renderer, concurrent: true, showErrorMessage: false, rendererOptions};
    const runner = new Manager(listrOptions);
    runner.add(tasks);
