...
module.exports = class Cli {
  ...
  defaultConfig(appConfig = {}) {
    ...
    // the path invoked to start the cli
    const entrypoint = packaged ? process.execPath : process.argv[1];
    // the actual file being used to invoke the cli
    const bin = _.get(process, 'mainModule.filename', entrypoint);
    const binPath = _.get(process, 'mainModule.path', path.dirname(entrypoint));
    ...
    const commit = require('../utils/get-commit-hash')(srcRoot, {short: true});

    // put it all together
    const cli = {bin, commit, entrypoint, installPath, packaged, path: binPath, plugin: srcRoot, source};

    console.log(cli);
    process.exit(1)

    // @TODO:
    // 1. if packaged dev release and corebase is internal then get version from package.json?
    //    v3.20.4-X-gFOUR

    // @NOTE: as of 3.21 the version comes from core, not the cli
    const version = require(path.join(this.coreBase, 'package.json')).version;
