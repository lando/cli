...
module.exports = class Cli {
  ...
  defaultConfig(appConfig = {}) {
    ...
    const entrypoint = packaged ? process.execPath : _.get(process, 'mainModule.filename', process.argv[1]);
    // this should be the symlink resolved dir where the "entrypoint" is installed
    const installPath = path.dirname(entrypoint);
    ...
    // put it all together
    const cli = {commit, coreBase, dev, entrypoint, installPath, packaged, plugin: srcRoot, source};
