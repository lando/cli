...
module.exports = class Cli {
  ...
  defaultConfig() {
    ...
    const pluginsRoot = path.resolve(srcRoot, 'plugins');
    const version = require(path.join(__dirname, '..', 'package.json')).version;
    const pluginDirs = [
      srcRoot,
      {path: path.join(pluginsRoot, 'core'), subdir: '.'},
      {path: path.join(pluginsRoot, 'services'), subdir: '.'},
      {path: path.join(pluginsRoot, 'recipes'), subdir: '.'},
      {path: path.join(pluginsRoot, 'integrations'), subdir: '.'},
      {path: path.join(pluginsRoot, 'external'), subdir: '.'},
      this.userConfRoot,
    ];
